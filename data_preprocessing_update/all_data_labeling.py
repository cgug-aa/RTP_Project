...
while grid_queue:
    ...
    if grid_size_km > min_size_km and is_path_in_grid(south, west, north, east, all_path_point):
...
grid_df.to_csv('all1_grid_information_with_paths.csv', index=False)
...
#    df.to_csv(f'\\data\\Label\\{labeled_file}.csv', index=False)

# Save or show map
m.save('all1_south_korea_grid_map_with_subdivided_paths.html')